...
if docker ps --format '{{.Names}}' | grep -qx 'frappe-db-1'; then
  docker exec frappe-db-1 mariadb-dump -uroot -p123 --all-databases > "${target_dir}/prod/prod-frappe-db.sql"
fi
...
tar -czf "${target_dir}/system/infra-config.tgz" -C "/opt/velveta" infra
...
if [ -d /home/erpdev/.ssh ]; then
  tar -czf "${target_dir}/system/erpdev-ssh.tgz" -C /home/erpdev .ssh
fi

if [ -f /etc/systemd/system/actions.runner.ibober89-ecommerce.vm519642.service ]; then
  cp /etc/systemd/system/actions.runner.ibober89-ecommerce.vm519642.service "${target_dir}/system/"
fi
