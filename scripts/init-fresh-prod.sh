...
docker compose -f "${FRAPPE_COMPOSE_FILE}" exec -T backend \
  bash -lc "test -f sites/${SITE_NAME}/site_config.json && (bench --site ${SITE_NAME} list-apps | awk '{print \$1}' | grep -qx blog || bench --site ${SITE_NAME} install-app blog) && bench --site ${SITE_NAME} migrate && bench --site ${SITE_NAME} execute ecommerce.patches.sync_sales_print_formats_v7.execute && bench --site ${SITE_NAME} execute ecommerce.patches.ensure_hepsijet_shipping_rates.execute && bench --site ${SITE_NAME} clear-cache"
